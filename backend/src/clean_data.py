...
def normalize_periodo_academico(df: pd.DataFrame) -> pd.DataFrame:
    """Normaliza periodos académicos a categorías estándar"""
    mapping = {
        "semestre_1": "SEM_1",
        "SEMESTRE_1": "SEM_1",
        "Semestre_1": "SEM_1",
...
def fix_timestamp(df: pd.DataFrame) -> pd.DataFrame:
    """Convierte timestamp y extrae features temporales"""
...
def recal_energia_total(df: pd.DataFrame) -> pd.DataFrame:
    """Recalcula energía total sumando componentes por sector"""
